"""OpenAI Agents SDK runtime for the `/analyze` endpoint."""
...
class OpenAIAnalysisRuntime:
    ...
    async def run(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        result = await runner.run(agent, _build_analysis_input(context), max_turns=max_turns)

        final_output = getattr(result, "final_output", "")
        if not isinstance(final_output, str):
            final_output = str(final_output)

        return _normalize_analysis_output(_parse_json_output(final_output))
